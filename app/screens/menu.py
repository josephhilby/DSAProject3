...
class MenuScreen:
    ...
    def render(self):
        ...
        if self.alg_one_dropdown is None:
            self.alg_one_dropdown = pygame_gui.elements.UIDropDownMenu(relative_rect=pygame.Rect(SCREEN_WIDTH * 0.55, SCREEN_HEIGHT * 0.5, 100, 50),
                                                                       options_list=[algorithm.name for algorithm in algorithms],
                                                                       starting_option=algorithms[0].name,
                                                                       manager=self.manager)

        if self.alg_two_dropdown is None:
            self.alg_two_dropdown = pygame_gui.elements.UIDropDownMenu(relative_rect=pygame.Rect(SCREEN_WIDTH * 0.35, SCREEN_HEIGHT * 0.5, 100, 50),
                                                                       options_list=[algorithm.name for algorithm in algorithms],
                                                                       starting_option=algorithms[0].name,
                                                                       manager=self.manager)
